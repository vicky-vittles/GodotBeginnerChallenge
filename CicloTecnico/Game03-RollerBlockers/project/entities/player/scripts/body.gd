...
onready var up_collision_shape = $Up/CollisionShape
onready var down_collision_shape = $Down/CollisionShape


func input(dir: Vector3):
	if dir.x == 1:
...
func move(delta):
	if is_turning:
		...
		match rotation_direction:
			Direction.RIGHT:
				transform = transform.rotated(Vector3(0, 0, -1), angle)
			Direction.DOWN:
...
func start_turning(direction):
	if interpolation != 0 or not $GravityTimer.is_stopped():
		return
	is_turning = true
	match direction:
		Direction.RIGHT:
...
func adjust_transform(shift):
	get_parent().translation += translation + shift
	translation = -shift
