...
func bounce_scale(start_size: Vector2 = Vector2.ONE, end_size: Vector2 = Vector2(1.2, 1.2), time: float = 0.3):
	if not is_active: return
	tween.interpolate_property(self, "scale", start_size, end_size, time, Tween.TRANS_BOUNCE, Tween.EASE_IN_OUT)
	tween.start()
